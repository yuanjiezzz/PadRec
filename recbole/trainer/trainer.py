...
class Trainer(AbstractTrainer):
    def __init__(self, config, model):
        ...
        self.num_interactions_to_add = config["num_interactions_to_add"]
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None, show_progress=False, dataset=None):
        self.dataset = train_data.dataset
        candidate_user_id_tail, candidate_user_id_popular, max_matrix_dense = self.perform_epoch_start_operations(train_data.dataset)
        self.model.train()
        ...
        for batch_idx, interaction in enumerate(iter_data):
            ...
            with torch.autocast(device_type=self.device.type, enabled=self.enable_amp):
                losses = loss_func(interaction, candidate_user_id_tail, candidate_user_id_popular, max_matrix_dense)
    ...
    def normalize_sparse_matrix(self, sparse_matrix):
        row_indices = sparse_matrix.indices()[0]
        values = sparse_matrix.values()
        row_sums = torch.zeros(sparse_matrix.size(0), device=sparse_matrix.device)
        row_sums.index_add_(0, row_indices, values)
        row_sums[row_sums == 0] = 1.0
        normalized_values = values / row_sums[row_indices]
        normalized_matrix = torch.sparse_coo_tensor(sparse_matrix.indices(), normalized_values, sparse_matrix.size(), device=sparse_matrix.device)
        normalized_matrix = normalized_matrix.to_sparse_csr()
        
        return normalized_matrix


    #choose tail user and popular user based on frequency
    def perform_epoch_start_operations(self, dataset=None): 
        user_id_counts = dataset.user_counter

        M = dataset.inter_matrix(form='coo')
        values = torch.tensor(M.data, dtype=torch.float32)
        indices = torch.tensor(np.vstack((M.row, M.col)), dtype=torch.long)
        size = M.shape
        sparse_tensor = torch.sparse_coo_tensor(indices, values, size)
        sparse_tensor_M3 = sparse_tensor.to(self.device)
        MT = sparse_tensor_M3.T

        M3 = torch.sparse.mm(torch.sparse.mm(sparse_tensor_M3, MT), sparse_tensor_M3)
        M5 = torch.sparse.mm(torch.sparse.mm(M3, MT), sparse_tensor_M3)

        M3_scaled = M3 * 0.5
        M5_scaled = M5 * 0.25

        M3_clipped = M3_scaled
        M5_clipped = M5_scaled

        max_matrix_3 = M3_clipped
        max_matrix_5 = M5_clipped

        normalized_max_matrix_3 = self.normalize_sparse_matrix(max_matrix_3)
        normalized_max_matrix_5 = self.normalize_sparse_matrix(max_matrix_5)

        user_counts = torch.tensor(list(user_id_counts.values()), dtype=torch.float, device=self.device)
        user_priorities = 1.0 / user_counts
        # normolize
        user_probabilities = user_priorities / torch.sum(user_priorities) # tail user_probabilities
        user_probabilities_2 = user_counts / torch.sum(user_counts) # popular user_probabilities

        user_ids_list = list(user_id_counts.keys())
        selected_user_ids = []
        selected_user_ids_2 = []

        # nominate users
        if self.num_interactions_to_add:
            user_indices = torch.multinomial(user_probabilities, self.num_interactions_to_add, replacement=True) # tail user nomination index
            user_indices_2 = torch.multinomial(user_probabilities_2, self.num_interactions_to_add, replacement=True) # popular user nomination index
            selected_user_ids = [user_ids_list[index] for index in user_indices] # tail user ID
            selected_user_ids_2 = [user_ids_list[index] for index in user_indices_2] # popular user ID
        return selected_user_ids, selected_user_ids_2, normalized_max_matrix_3
        # return selected_user_ids, selected_user_ids_2, normalized_max_matrix_5
    ...
    def fit(
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            train_loss = self._train_epoch(
                train_data, epoch_idx, show_progress=show_progress, dataset=dataset
            )
    ...
    def _full_sort_batch_eval(self, batched_data):
        ...
        try:
            ...
            # breakpoint()
            scores = self.model.full_sort_predict(interaction.to(self.device), candidate_user_id_tail, candidate_user_id_popular, max_matrix_dense)

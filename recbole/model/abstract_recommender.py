...
class AutoEncoderMixin(object):
    ...
    def get_rating_matrix(self, user):
        ...
        # breakpoint()
        col_indices = self.history_item_id[user].flatten() # store the item ID of the user's inter history
        # torch.sum(self.history_item_id[user].flatten())
        ...
        # breakpoint()
        # rating_matrix.flatten().nonzero().squeeze() # find the index of NON zero items

        # 列的数值为一的索引 代表的是 那个item ID 有过交互 这里的ID 不是原始数据的 但是是内部封装好的
